...
echo Version 2.5.0
echo + Many Bug Fixes
echo + Added Bootloader Support
echo + Supplementary Updates can be downloaded through the 3rd Party Store.
echo + Revamped Setup for Windows 10 Threshold 2 or higher. If running anything less than Windows 10, it will default to the old version.
pause >nul
goto desktop
:3PSTORE
cls
...
if ERRORLEVEL 1 (
	...
	setlocal EnableDelayedExpansion
	echo %App% | findstr /i /c:"MyOS_Suplementary"
	if !ERRORLEVEL! == 0 (
		endlocal
		cls
		setlocal EnableDelayedExpansion
		echo %App% | findstr /i /c:%version% && (
			endlocal
			cls
			setlocal EnableDelayedExpansion
			echo This is a suplementary update of MyOS. This suplementary update is not required, but can be installed. It is compatible with the version of MyOS installed on this system. Install?
			choice /C YN /N
			if !ERRORLEVEL! == 1 (
				cls
				echo Downloding...
				bitsadmin /transfer myDownloadJob https://github.com/Duckloveshack/MyOS3PStore/raw/master/%App%.bat "%cd%\%App%-Installer.bat" >nul
				echo Installing Suplementary Update...
				start %App%-Installer.bat
				echo The Part 1 of the update is installed. Re-run MyOS to install it.
				# I think the next suplimentary update will include a patchID = 2.5.1 variable
				endlocal
				timeout /t -1 /nobreak >nul
			) else (
				endlocal
			...
			endlocal
			cls
			echo This suplementary update is not compatible with this version of MyOS.
	...
	if exist %App%.bat (
		...
		bitsadmin /transfer myDownloadJob https://github.com/Duckloveshack/MyOS3PStore/raw/master/%App%.bat "%cd%\%App%.bat" >nult
		pause
		echo Launching %App%.bat
		timeout /t 1 /nobreak >nul
		start %App%.bat
...
echo MyOS PatchID: %PatchID%
echo MyOS Bootloader Compatibility: Not Compatible
echo System Type: %OS%
...
cls
echo Welcome to MyOS %version% ! Here, you can choose the type of MP3 Player that is needed for you.
